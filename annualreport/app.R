...
#Shiny App for populating the annual report stats
#Author: Farshad Ebrahimi, Last updated: 10/4/2022
...
#connection 
con <- dbConnect(odbc::odbc(), dsn = "mars14_data", uid = Sys.getenv("shiny_uid"), pwd = Sys.getenv("shiny_pwd"))
...
server <- function(input, output) {
    ...
    table_31 <- reactive({
      ...
      sql_string_1 <- "select count(*) from fieldwork.viw_deployment_full_cwl
                                  ...
                                  and public =  TRUE"
      table_3_1_public_sensors_deployed_postcon <- dbGetQuery(con, paste(sprintf(sql_string_1, FYSTART_reactive(),FYSTART_reactive(), FYEND_reactive()),collapse="")) 
      ...
      sql_string_2 <- "select count(distinct admin.fun_smp_to_system(d.smp_id)) from fieldwork.viw_deployment_full_cwl d
                                            ...
                                            and d.public = true"
      table_3_1_public_systems_monitored <- dbGetQuery(con, paste(sprintf(sql_string_2, FYSTART_reactive(), FYEND_reactive(), FYEND_reactive()),collapse="")) 
    ...
    table_310 <- reactive({
      ...
      sql_string_25 <- "select count(distinct admin.fun_smp_to_system(d.smp_id)) from fieldwork.viw_deployment_full_cwl d
                                  ...
                                  and (collection_dtime_est >= '%s'
                                      or collection_dtime_est is null) 
                                  and d.public = false"
      table_3_10_private_systems_monitored <- dbGetQuery(con, paste(sprintf(sql_string_25, FYSTART_reactive(), FYEND_reactive(), FYSTART_reactive()),collapse="")) 
