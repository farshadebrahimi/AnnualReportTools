...
## queries to gets stats
  ...
  sql_string <- "select count(*) from fieldwork.deployment_full_cwl
                                  ...
                                  and public =  TRUE"
  table_3_1_public_sensors_deployed_postcon <- dbGetQuery(con, paste(sprintf(sql_string, FYSTART, FYSTART, FYEND),collapse="")) 
  ...
  #First column, final row: This fiscal year, total
  
  sql_string <- "select count(*) from fieldwork.srt_full 
                                      where test_date >= '%s'
                                      and test_date <= '%s'
                                      and phase = 'Post-Construction'
                                      and public = TRUE"
  
  table_3_3_public_system_postcon_srt_fy_total <-dbGetQuery(con, paste(sprintf(sql_string, FYSTART, FYEND),collapse=""))
  
  
  #Post-construction SRTs performed on Public Systems TO DATE
  #Second column, final row: The above, to date, total
  ...
  table_3_3_public_system_postcon_srt_todate_total <- dbGetQuery(con, paste(sprintf(sql_string,FYEND),collapse=""))
  ...
  #First column, last row: This fiscal year total SRTs
  ...
  table_3_7_public_midcon_srt_fy_total <- dbGetQuery(con, paste(sprintf(sql_string, FYSTART, FYEND),collapse=""))
  ...
  table_3_7_public_midcon_srt_todate_total <- dbGetQuery(con, paste(sprintf(sql_string, FYEND),collapse=""))
  ...
  sql_string <- "select count(distinct smp_to_system(d.smp_id)) from fieldwork.deployment_full_cwl d
                                  ...
                                  and (collection_dtime_est >= '%s'
                                      or collection_dtime_est is null) 
                                  and d.public = false"
  table_3_10_private_systems_monitored <- dbGetQuery(con, paste(sprintf(sql_string, FYSTART,FYEND, FYSTART),collapse="")) 
  ...
  #3.4.2: Private SRTs
  #Table 3.12: Post-construction private SRTs
  #First column final row: Total SRTs 
  sql_string <-"select count(*)
                          from fieldwork.srt_full 
                          where test_date >= '%s'
                          and test_date <= '%s'
  ...
  table_3_12_private_postcon_srt_fy_total <- dbGetQuery(con, paste(sprintf(sql_string,FYEND),collapse=""))
  
  #3.4.2: Private SRTs
  #Table 3.12: Post-construction private SRTs
  #Second column final row: Total SRTs 
  sql_string <-"select count(*)
                          from fieldwork.srt_full 
                          where test_date <= '%s'
                          and phase = 'Post-Construction'
  ...
  table_3_12_private_postcon_srt_todate_total <- dbGetQuery(con, paste(sprintf(sql_string,FYEND),collapse=""))
